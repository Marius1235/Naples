...
import { faArrowAltCircleDown, faArrowAltCircleLeft, faArrowAltCircleRight, faArrowAltCircleUp, faImagePortrait, faImage } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
...
const BackgroundChoiceComponent: React.FC = () => {
  ...
  return (
    ...
    const [selectedBackground, setSelectedBackground] = useState<string>(require("../assets/images/Background1Landscape.jpg"));
    const canvasRef = useRef<HTMLCanvasElement | null>(null);
    const capturedImageContext = useContext(CapturedImageContext);
    const [position, setPosition] = useState({ x: 0, y: 0 });
    const navigate = useNavigate();
            ...
            <div className="col-sm-2" id="bg-selector">
                <div className="background-bar">
                    {backgroundOptions.map((option: IImage) => (
                        ...
                        src={option.imageUrl} alt={option.alt} onClick={() => changeBackground(option.landscapeUrl)}
                        />
                    ))}
                </div>
            </div>
